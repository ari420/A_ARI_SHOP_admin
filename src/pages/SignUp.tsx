...
const SignUp: React.FC = () => {
  ...
  // Check and create user if form is submitted
  useEffect(() => {
    ...
    const checkAndCreateUser = async () => {
      ...
      const { name, gmail, password } = formData;
      console.log("Form Data:", formData);

      try {
        // Construct the URL to check if user exists
        const url = `${baseURL}?gmail=${gmail}`;
        ...
        if (existingUsers.length === 0) {
          // Create new user
          const newUser = { name, gmail, password, user: "user" };
          ...
          if (!postResponse.ok) {
            throw new Error(
              `Failed to create user. Status: ${postResponse.status}`
            );
          }

          const newUserResponse = await postResponse.json();
          ...
          if (gmailRef.current) gmailRef.current.value = "";
  ...
  return (
    <main className=" pt-10 xl:p-12 2xl:p-30  flex justify-center items-center">
